...
enum tnep_state_name current_state = TNEP_STATE_DISABLED;
...
int nfc_tnep_event_offset[NFC_TNEP_EVENTS_NUMBER];
struct k_poll_signal nfc_tnep_msg_rx;
struct k_poll_signal nfc_tnep_msg_tx;
...
int nfc_tnep_init(struct nfc_tnep_service *services, size_t services_amount)
{
	...
	k_poll_signal_init(&nfc_tnep_timer_signal);
	...
	K_POLL_TYPE_SIGNAL,
			  K_POLL_MODE_NOTIFY_ONLY, &nfc_tnep_timer_signal);
