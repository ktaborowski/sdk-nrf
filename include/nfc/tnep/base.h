...
#ifndef NFC_TNEP_BASE_H__
#define NFC_TNEP_BASE_H__
...
#define NFC_TNEP_MASKED_VALUE(x, mask) ((x > mask)?mask:(x & mask))

#define NFC_TNEP_MAX_EXEC_MASK (0x0fU)
#define NFC_TNEP_MAX_EXEC_NO(N_wait) NFC_TNEP_MASKED_VALUE(N_wait,\
...
#define NFC_TNEP_WT_INT(wt_init) NFC_TNEP_MASKED_VALUE(wt_init,\
...
struct nfc_tnep_service {
	struct nfc_ndef_tnep_svc_param *parameters;
...
#define NFC_TNEP_SERVICE_DEF(name, uri, uri_length,			\
	...
	static const struct nfc_tnep_service_cb name##_cb = {		\
		.selected = select_cb,					\
